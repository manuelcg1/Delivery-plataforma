...
export function MerchantProvider({children}:{children:React.ReactNode}){const{me}=useAuth();const[context,setContext]=useState<MerchantContext|null>(null);const[merchantId,setMerchant]=useState('');const[branchId,setBranch]=useState('');
 const reload=useCallback(async()=>{const value=await api<MerchantContext>('/api/v1/merchant/context');setContext(value);const merchant=value.merchants.find(x=>x.id===merchantId)??value.merchants[0];setMerchant(merchant?.id??'');setBranch(validBranchId(merchant,branchId))},[merchantId,branchId]);
 useEffect(()=>{if(me)reload().catch(()=>setContext({userId:me.id,tenantId:'',merchants:[]}))},[me,reload]);
 const setMerchantId=useCallback((id:string)=>{setMerchant(id);setBranch('')},[]);
