...
public class CustomerEngagementController {
    ...
    @PostMapping("/orders/{orderId}/rating") @ResponseStatus(HttpStatus.CREATED) @Transactional Rating rate(@AuthenticationPrincipal IdentityPrincipal p,@PathVariable UUID orderId,@Valid @RequestBody RatingRequest q){ownedOrder(p,orderId,true);db.sql("insert into order_ratings(tenant_id,order_id,customer_id,score,comment) values(:tenant,:order,:user,:score,:comment) on conflict(order_id,customer_id) do update set score=excluded.score,comment=excluded.comment").param("tenant",p.tenantId()).param("order",orderId).param("user",p.userId()).param("score",q.score()).param("comment",q.comment()).update();return rating(p,orderId);}
