...
class CheckoutCoverageTest {
    @Test void rejectsCoverageOutsideConfiguredZone(){OrdersRepository repo=mock(OrdersRepository.class);DeliveryCoverageService coverage=mock(DeliveryCoverageService.class);OrdersService service=new OrdersService(repo,coverage,new SimpleMeterRegistry());UUID tenant=UUID.randomUUID(),customer=UUID.randomUUID(),merchant=UUID.randomUUID(),branch=UUID.randomUUID(),address=UUID.randomUUID();IdentityPrincipal principal=new IdentityPrincipal(customer,tenant,"tenant",Set.of(),Set.of());Cart cart=new Cart(UUID.randomUUID(),customer,merchant,branch,new BigDecimal("20"),BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal("20"),"PEN","ACTIVE",List.of(),Instant.now(),Instant.now());when(repo.activeCart(tenant,customer)).thenReturn(Optional.of(cart));when(coverage.quote(tenant,customer,merchant,branch,address,new BigDecimal("20"))).thenReturn(new DeliveryCoverageService.Quote(false,null,null,0,null,null,null,"DELIVERY_OUT_OF_COVERAGE","Dirección fuera de cobertura"));var result=service.coverage(principal,merchant,address);assertThat(result.covered()).isFalse();assertThat(result.message()).contains("todavía no realiza entregas");verify(repo).address(tenant,customer,address);}
}
