...
class CheckoutCoverageTest {
    @Test void rejectsCoverageOutsideConfiguredZone(){OrdersRepository repo=mock(OrdersRepository.class);DeliveryCoverageService coverage=mock(DeliveryCoverageService.class);OrdersService service=new OrdersService(repo,coverage,new SimpleMeterRegistry());UUID tenant=UUID.randomUUID(),customer=UUID.randomUUID(),merchant=UUID.randomUUID(),branch=UUID.randomUUID(),address=UUID.randomUUID();IdentityPrincipal principal=new IdentityPrincipal(customer,tenant,"tenant",Set.of(),Set.of());Cart cart=new Cart(UUID.randomUUID(),customer,merchant,branch,new BigDecimal("20"),BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,new BigDecimal("20"),"PEN","ACTIVE",List.of(),Instant.now(),Instant.now());when(repo.activeCart(tenant,customer)).thenReturn(Optional.of(cart));when(coverage.quote(tenant,customer,merchant,branch,address,new BigDecimal("20"))).thenReturn(new DeliveryCoverageService.Quote(merchant,branch,address,null,null,null,null,null,null,null,null,false,false,false,"DELIVERY_NOT_COVERED","Este comercio todavía no realiza entregas en esta ubicación.","PEN",BigDecimal.ZERO,30,25,35,10,5,5,10,Instant.now()));var result=service.coverage(principal,merchant,address);assertThat(result.covered()).isFalse();assertThat(result.reasonCode()).isEqualTo("DELIVERY_NOT_COVERED");assertThat(result.message()).contains("todavía no realiza entregas");verify(repo).address(tenant,customer,address);}
}
